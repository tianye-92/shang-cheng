$(document).ready(function(){
	...
	$("#modifySavePassword").click(function(){
		...
		if("" == oldpwd){
			...
			$.ajax({
				...
				data:{upUser:JSON.stringify(user)},
				dataType: "text",
				timeout: 1000,
				error: function(){
					alert("修改密码失败！请重试。");
				...
				success: function(result){
					if(result != "" && "success" == result){
						...
						window.location.href='/user/xiugaimima';
	...
	/**----------菜单列表的修改密码*/
	$("#modifypwstianye").click(function(){
		var tip = $("#modifypwdtiptianye");
		tip.html("");
		var oldpwd = $("#oldpasswordtianye").val();
		var newpwd = $("#newpasswordtianye").val();
		var aginpwd = $("#aginpasswordtianye").val();
		if("" == oldpwd){
			tip.css("color","red");
			tip.html("对不起，请输入原密码，谢谢。");
			$("#oldpasswordtianye").focus();
		}else if(newpwd.length < 6){
			tip.css("color","red");
			tip.html("对不起，新密码长度不能小于6位，谢谢。");
			$("#newloginpwdtianye").focus();
		}else if("" == aginpwd){
			tip.css("color","red");
			tip.html("对不起，请再次输入新密码，谢谢。");
			$("#aginpasswordtianye").focus();
		}else if(newpwd != aginpwd){
			tip.css("color","red");
			tip.html("对不起，您两次输入的密码不一致，请重新输入，谢谢。");
			$("#aginpasswordtianye").focus();
			...
			$.ajax({
				...
				success: function(result){
					if(result != "" && "success" == result){
						alert("修改密码成功，请使用新密码重新登录！");
						window.location.href='/user/zhuXiao';
					}else if("failed" == result){
						tip.css("color","red");
						tip.html("修改密码失败！请重试。");
					}else if("oldpwdwrong" == result){
...
//    $('#reply_createTime').val((d.getMonth()+1)+'/'+d.getDate()+'/'+d.getFullYear());
	...
	for(var i = 0;i<json.length;i++){
		...
		//config main menu
		result = result + '<li class="nav-header hidden-tablet" onclick="$(\'#test'+i+'\').toggle(500);" style="cursor:pointer;">'+json[i].zhuMenu.functionName+'</li>';
		//config sub menus
		result = result + "<li><ul class=\"nav nav-tabs nav-stacked\" id=\"test"+i+"\">";
		
		for(var j=0;j<json[i].ziMenus.length;j++){
			var pic;
			switch(j)
			{
			case 0:
			...
			result = result + "<li><a class=\"ajax-link\" style=\"cursor:pointer;\" href=\""+json[i].ziMenus[j].funcUrl +"\"><i class="+pic+"></i><span class=\"hidden-tablet\">"+json[i].ziMenus[j].functionName + "</span></a></li>";
		}
		result = result +"</ul></li>";
	}
	$("#menus").append(result);
